...
prep_ko<-function(data, location_set_id=22, start_year=1980, end_year=2024, by_sex=1, by_age=1, release_id=release_id){
  if(F){
    location_set_id=22
    start_year=1980
    end_year=2024
  ...
  print(dim(data))
  ##sy: get level from location metadata if it isn't already there
  ...
  ##sy: if there are multiple sexes in the data, by_sex should be 0 for now
  ...
  ##sy:get square, the function in db_tools script.  The start and end year args don't really matter- if they aren't in the data, they won't be held out.
  ##sy: if 1 and 2 are in sex_id, by_sex needs to be 0.  If unique sex_id is 3, then by sex should be 0
  ...
  print(dim(no_na_data))
  print(dim(sqr))
  sqr[, sex_id:=NULL]
  # TODO: is this not working? getting st-gpr error saying that custom stage 1 prior is not square (missing loc-age-years compared with gbd square)
  # May be a years issue. How/where is the ST-GPR square year range defined? A: stgpr_helpers repo,
  # https://stash.ihme.washington.edu/projects/CCGMOD/repos/stgpr_helpers/browse/src/stgpr_helpers/lib/prep_utils.py#61
  ...
  ##sy: get all locations
  locations1<-unique(data.sqr$location_id)
  
  ##sy: get locations w/ data
...
get_kos<-function(square_data, no_miss_data, all_locs, data_locs, kos=5, prop_to_hold=.20, no_new_ages=T, only_data_locs=F, seed=32, drop_nids=T){
  ...
  ##sy: can overwrite all locations to include at least some data.. this avoids dropping 100% of the data in a location
  if(only_data_locs==T){
    all_locs<-data_locs
  }
  ...
  for(ko in 1:kos){
    ...
    ##sy: bind together the missing data and the non-missing data.
    ...
    ##sy: this while loop gets data_ids to drop until the specified proportion is hit
    loc_vect<-sample(unique(data_to_ko[test1==T, loc2]), size=length(unique(data_to_ko[test1==T, loc2])), replace=F)
    ids_to_drop<-c()
    i<-0
    # TODO: error: missing value where TRUE/FALSE needed
    while(length(unique(ids_to_drop))/nrow(no_na_data)<prop_to_hold){ i<-i+1;
    
    ##sy: need to write this check in for PJ's diet data, replicated age-year patterns in FAO data
    if(i>length(loc_vect)){
      ...
      ##sy: this is the normal KO process
      loc<-loc_vect[i]
    ...
    #message("  ", round(sum(no_na_data[,get(paste0("ko", ko))])/nrow(no_na_data)*100, digits=3), "% of data held out for ko ", ko)
    
    ##sy: save some metadata at some point
    #locs_dropped<-no_na_data[, .(num_dropped=sum(paste0("ko", ko)))]
    
    ##sy: need to check if an entire age group id gets dropped.. this is only a problem if factor(age_group_id) is in the model, it will break trying to predict new levels of a fixed effect
    if(no_new_ages==T){
      ...
      if(length(all_held_ages>1)){
        ...
        ##sy: put data points back into the training dataset
