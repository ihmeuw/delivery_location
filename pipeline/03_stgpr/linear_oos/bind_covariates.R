...
bind_covariates<-function(df, cov_list, custom_cov_list=NULL, release_id=16){
  ...
  if(!"location_id"  %in% names(df) & "ihme_loc_id"  %in% names(df)){
    ...
    locs<-get_location_metadata(location_set_id=22, release_id=release_id)[, .(ihme_loc_id, location_id)]
  ...
  for(i in 1:length(cov_list)){
    ...
    if(cov_list[i] %in% cust_covs){ ##sy: get covariate from custom location
      ...
      x<-get_covariate_estimates(covariate_id=cid, location_id=unique(df$location_id),
                                 ...
                                 sex_id=-1, release_id=release_id)
    }
