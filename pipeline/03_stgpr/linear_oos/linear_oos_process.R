...
for(index in start:end){ 
  ...
  output<-file.path(output_folder, paste0("form_", sexchar, "_", index, ".csv")) ##sy: save the output of each model here
  ...
  for(ko in 0:kos){ 
    ...
    if(ko==0){
      test<-data  ##sy: this is in sample rmse.  Same as train but includes all subnats
      train<-data
    }
